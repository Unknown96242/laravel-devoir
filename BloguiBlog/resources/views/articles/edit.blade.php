...
@section('content')
    <div class="max-w-2xl mx-auto px-4 py-8">
        ...
        <form action="{{ route('articles.update', $article) }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <textarea
                    ...
                    placeholder="Rédigez le contenu de votre article">{{ old('contenu', $article->contenu) }}</textarea>
                @error('contenu')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
